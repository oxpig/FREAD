    ...
    try:
        if not just_get_the_best_esss:
            ...
            WHERE
              ...
              AND score >= ?
            """, (casep1, casep1, casep2, casep2, casep3, casep3, casep4, casep4, min_casep1, max_casep1, min_casep2,
                  max_casep2, min_casep3, max_casep3, min_casep4, max_casep4, strict_rmsd_cutoff ** 2, score_cutoff))
        else:
            query = conn.execute("""
            SELECT
              score_seq(sequence, dihedral) AS score,
...
def relabel_loop(decoy_residues, sequence, startnum=0, endnum=0, endinscode="", chain="A", prevatom=None,
                 nextatom=None):
