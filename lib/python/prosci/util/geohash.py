...
class GeometricHash:
    ...
    def get_neighbours(self, c, num, max_dist=None, filter=None):
            ...
            if L is not None:
              for i in L:
                ...
                if not max_dist or val[0] < max_dist:
                  ...
                  if index < num:
                    if filter is None or list(filter(result, val)):
        ...
        while len(result) < num or result[-1][0] > self.sqr_cell_size * d * d:
            ...
            for x in range(max(0, ix[0]-d), min(self.dimensions[0], ix[0]+d+1)):
                
                # Two opposing sides of a cube
                for y in range(max(0, ix[1]-d), min(self.dimensions[1], ix[1]+d+1)):
                    if ix[2]-d > 0:
                        save((x, y, ix[2]-d))
                ...
                for z in range(max(0, ix[2]-d+1), min(self.dimensions[2], ix[2]+d)):
                    if ix[1]-d > 0:
                        save((x, ix[1]-d, z))
                    if ix[1]+d < self.dimensions[1]:
                        save((x, ix[1]+d, z))
                    
            for y in range(max(0, ix[1]-d+1), min(self.dimensions[1], ix[1]+d)):
